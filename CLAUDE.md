...
| 2026-06-26 | Story 2.2 DONE (login/logout JWT sessions) | bdsai/ apps/api + apps/web + packages/shared | User login/logout with JWT. NestJS: POST /auth/login (Throttle 5/15min), POST /auth/logout, POST /auth/refresh, GET /auth/me (JwtAuthGuard). Hybrid JWT verify: HS256 local (fast) → ES256 fallback (supabase.auth.getUser — new Supabase v2.188+ uses asymmetric keys). Refresh token httpOnly cookie (Secure, SameSite=Lax, Path=/api/auth, 7d). Anti-enumeration: generic "Email hoặc mật khẩu không đúng". Web: /login page + LoginForm (Zod UX), AuthProvider (accessToken in memory, restore via refresh on mount), useAuthFetch (401→auto refresh→retry), HeaderAuthActions (conditional), 4 API thin proxies. 79 unit + 18 web test pass. Real curl verify 7/7 AC (Supabase + Redis thật). Build/lint/typecheck PASS |
| 2026-06-26 | Fix 2 review findings Story 2.2 (HIGH-1 + MEDIUM-1) | bdsai/apps/api/src/auth | HIGH-1: Logout dùng jwt.decode (không verify signature) → DoS. Fix: thêm JwtAuthGuard cho POST /auth/logout, service nhận userId từ req.user.id (guard đã verify). E6: no-token → 401 (guard reject), signOut fail → vẫn 200. MEDIUM-1: JwtAuthGuard không validate iss. Fix: check payload.iss === ${SUPABASE_URL}/auth/v1 sau jwt.verify (HS256 path), reject 401 nếu mismatch. 80 unit + 18 e2e pass. Curl verify 6/6 (logout 200 valid JWT, 401 modified JWT, 401 no-token, me 200 correct iss, 401 wrong iss). Status → review |
