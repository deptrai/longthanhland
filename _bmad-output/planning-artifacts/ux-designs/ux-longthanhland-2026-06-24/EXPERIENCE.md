...
- **listing-card** (visual `{components.listing-card}`): click → detail; trust-badge overlay góc ảnh; lazy-load ảnh (NFR4).
...
- **filter-bar** (browse): location/price/type/area; áp filter không reload trang (client island), URL phản ánh filter (shareable + SEO).
